...
userAuthController.login = ("/user/login", async (req, res)=>{
    try {
        ...
        if(!user.isEmailVerified){
            ...
            //send response
            utilities.setResponseData(res, 401, {'content-type': 'application/json'}, {statusCode: 401, responseData: {msg: "Unverified email", userId: user._id}}, true)
            //SEND OTP TO EMAIL
        ...
        //create token
        const token = utilities.jwt("sign", {userID: user._id, role: user.role})
        ...
        //send response
        utilities.setResponseData(res, 200, {'content-type': 'application/json'}, {statusCode: 200, responseData:{user}}, true)
        return
